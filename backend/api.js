...
// Функция для MD5 (чтобы совпадало с твоей базой)
const md5 = (data) => crypto.createHash('md5').update(data).digest("hex");

// Подключение к MySQL (Railway подставит данные сам)
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      res.json({ success: false, error: 'Логин немесе құпиясөз қате' });
    }
  } catch (err) {
...
app.post('/api/objects', async (req, res) => {
  ...
  try {
    ...
    const [result] = await conn.execute(
      ...
      [name, description, layer_id, type, lat, lng, JSON.stringify(attributes || {})]
    );
    conn.release();
